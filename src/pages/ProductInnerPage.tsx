...
const ProductInnerPage = ({ onAddToCart, onClose }: ProductInnerPageProps) => {
  ...
  const allAttributesSelected =
    data?.product?.attributes?.every(attrSet => currentSelections[attrSet.id] !== undefined) ??
  ...
  if (!data || !data.product)
    return <p className="py-20 text-center text-xl">Product not found.</p>;
    ...
    <div className="ProductsInnerContainer container mx-auto mt-8 sm:mt-12 mb-20 px-4 sm:px-6">
      <div className="lg:grid lg:grid-cols-2 lg:gap-x-10 xl:grid-cols-[auto_1fr] xl:gap-x-12 items-start">
        ...
        <div className="mt-8 lg:mt-0 pr-[160px] space-y-5 md:space-y-6 lg:pl-15">
          <div>
            <h1 className="text-[30px] sm:text-3xl font-[600] text-[#1D1F22] tracking-normal">
              {product.name}
            </h1>
          </div>
          ...
          {currentPrice && (
            <div className="mt-6">
              <h3 className="text-[18px] font-[700] text-[#1D1F22] uppercase tracking-wider mb-1">
                Price:
              </h3>
          ...
          {product.description && cleanDescription && (
            <div className="mt-8">
              <div className="text-sm text-gray-700 space-y-3" data-testid="product-description">
